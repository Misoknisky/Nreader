...
class Vocab(object):
    ...
    """
    def __init__(self, filename=None, initial_tokens=None, lower=False,args=None):
        self.id2token = {}
        self.token2id = {}
        self.token_cnt = {}
        self.lower = lower
        ...
        self.embeddingpath=args.embedding[0]
        self.allembeddings=dict()
    ...
    def filter_tokens_by_cnt(self, min_cnt):
        ...
        for token in filtered_tokens:
            self.add(token, cnt=0)
    def initlist(self):
        for _ in range(len(self.id2token)):
            self.embeddings.append(0)
    def generatorEmbeddings(self,embed_dim):
        self.embed_dim = embed_dim
        self.initlist()
        len_num=0
        print("total",len(self.id2token))
        with open(self.embeddingpath,"r") as f:
            for line in f:
               content=line.split()
               try:
                  self.allembeddings[content[0]]=np.array(content[1:], dtype=np.float32)
               except BaseException:
                  pass
        for index in self.id2token.keys():
            if self.id2token[index] in [self.pad_token, self.unk_token]:
                self.embeddings[self.get_id(self.id2token[index])] = np.zeros([self.embed_dim])
            elif self.id2token[index] in self.allembeddings:
                self.embeddings[index]=self.allembeddings[self.id2token[index]]
            else:
                len_num +=1
                self.embeddings[self.get_id(self.id2token[index])] = np.random.rand(self.embed_dim)
        print("lacking",len_num)
