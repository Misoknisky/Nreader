...
from preprocess.dataset  import BRCDataset
from preprocess.vocab import Vocab
from model.basemodel.rc_model import RCModel
from model.models.rnet import RNET
from model.models.qanet import QANET
from model.models.ds_qa import DSQA
from model.models.Bidafplus import BIDAFPLUS
from model.models.bidafplus_ import BIDAFPLUSP
...
def parse_args():
    ...
    train_settings.add_argument('--weight_decay', type=float, default=0.0001,help='weight decay')
    ...
    model_settings.add_argument('--algo', choices=['BIDAF', 'MLSTM','VNET'], default='BIDAF',help='choose the algorithm to use')
    ...
    path_settings.add_argument('--embedding', nargs='+',default=['data/embeddings/vectors.txt'],help='list of files that contain the embedding vector')
    path_settings.add_argument('--train_files', nargs='+',default=['data/demo/trainset/train.json'],help='list of files that contain the preprocessed train data')
    path_settings.add_argument('--dev_files', nargs='+',default=['data/demo/devset/all.dev.json'],help='list of files that contain the preprocessed dev data')
    path_settings.add_argument('--test_files', nargs='+',default=['data/demo/testset/all.test.json'],help='list of files that contain the preprocessed test data')
...
def prepare(args):
    ...
    vocab = Vocab(lower=True,args=args)
    ...
    vocab.generatorEmbeddings(args.embed_size)
...
def train(args):
    ...
    rc_model = BIDAFPLUSP(vocab, args)
