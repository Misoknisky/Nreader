...
from model.layers.match_layer import GatedLayer
...
class RCModel(object):
    ...
    def _build_graph(self):
        ...
        self._match()# using MLSTM or BIDAF 
        self._fuse()# pass and question
        self._selfatt() 
        self._decode()# get start and end
        self._compute_loss()
    ...
    def _embed(self):
        ...
        with tf.device('/cpu:0'), tf.variable_scope('word_embedding'):
            self.word_embeddings = tf.get_variable(
                ...
                initializer=tf.constant_initializer(np.asarray(self.vocab.embeddings, dtype=np.float32)),
                trainable=True
            )
    ...
    def _encode(self):
        ...
        """
        highway_layer = GatedLayer(2 * self.hidden_size)
        with tf.variable_scope('passage_encoding'):
            self.sep_p_encodes, _ = rnn('bi-gru', self.p_emb, self.p_length, self.hidden_size)
            gated = highway_layer.gate(self.p_emb)
            self.sep_p_encodes = gated * self.sep_p_encodes + (1 - gated) * self.p_emb

        with tf.variable_scope('question_encoding'):
            self.sep_q_encodes, _ = rnn('bi-gru', self.q_emb, self.q_length, self.hidden_size)
            gated = highway_layer.gate(self.q_emb)
            self.sep_q_encodes = gated * self.sep_q_encodes + (1 - gated) * self.q_emb
        if self.use_dropout:
    ...
    def _fuse(self):
        ...
        with tf.variable_scope('fusion'):
            self.fuse_p_encodes, _ = rnn('bi-gru', self.match_p_encodes, self.p_length,self.hidden_size, layer_num=1)
    ...
    def _selfatt(self):
        ...
        with tf.variable_scope("document_attention"):
            ...
            self.document_encodes, _ = rnn('bi-gru', doc_att, None, self.hidden_size)
    def _decode(self):
        """
    ...
    def _train_epoch(self, train_batches,data, dropout_keep_prob,batch_size,max_bleu_4,max_rouge,pad_id,save_dir, save_prefix,evaluate):
        """
        ...
        return 1.0 * total_loss / total_num,max_bleu_4,max_rouge
    def train(self, data, epochs, batch_size, save_dir, save_prefix,dropout_keep_prob=1.0, evaluate=True):
        ...
        for epoch in range(1, epochs + 1):
            ...
            train_loss,max_bleu_4,max_rouge = self._train_epoch(train_batches, data,dropout_keep_prob,batch_size,max_bleu_4,max_rouge,pad_id,save_dir, save_prefix,evaluate)
        ...
        self.logger.info("max_Bleu-4 {},max_Rouge-L {}".format(max_bleu_4,max_rouge))
